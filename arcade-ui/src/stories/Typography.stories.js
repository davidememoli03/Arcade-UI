...
export default {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Tipografia ufficiale: Press Start 2P (display), VT323 (body), Share Tech Mono (dati/codice). ' +
          'Token da `--arc-font-*` e utilità `.arc-text-*`. Caricamento: woff2 self-hosted in `/fonts/` + Google Fonts.',
      },
    },
  },
}
...
const scale = [
  { cls: 'arc-text-display', token: '--arc-text-display', label: 'Display', sample: 'GAME OVER' },
  { cls: 'arc-text-h1',      token: '--arc-text-h1',      label: 'H1',      sample: 'INSERT COIN' },
  { cls: 'arc-text-h2',      token: '--arc-text-h2',      label: 'H2',      sample: 'High Scores' },
  { cls: 'arc-text-h3',      token: '--arc-text-h3',      label: 'H3',      sample: 'Player One' },
  { cls: 'arc-text-body',    token: '--arc-text-body',    label: 'Body',    sample: 'Press start to continue...' },
  { cls: 'arc-text-caption', token: '--arc-text-caption', label: 'Caption', sample: '© 1984 ARCADE INC' },
]

function scaleRow({ cls, token, label, sample }) {
  return `
    <div style="margin-bottom:2rem;border-bottom:1px solid rgb(0 119 119 / 35%);padding-bottom:1.25rem;">
      <div style="font-size:.65rem;color:var(--arc-color-text-muted);font-family:var(--arc-font-mono);margin-bottom:.5rem;">
        <strong>.${cls}</strong> · ${token}-*
      </div>
      <div class="${cls} arc-glow-cyan">${sample}</div>
    </div>`
}

/** Scala tipografica con classi utility `.arc-text-*`. */
export const Scale = {
  name: 'Scale (utilities)',
  render: () => `<div style="padding:2rem;max-width:640px;">${scale.map(scaleRow).join('')}</div>`,
}

/** Tre famiglie: display, body, monospace (anteprima diretta). */
...
/** `.arc-text-mono` per blocchi dati / diagnostica. */
export const MonoUtility = {
  name: 'Mono utility',
  render: () => `
...
/** Confronto rapido uppercase + numeri (typical HUD). */
export const ArcadeHudSample = {
